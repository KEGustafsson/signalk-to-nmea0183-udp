...
describe('schema', function () {
  ...
  it('defines array items as objects with sentence and throttle', function () {
    const items = conversionsProp(schema).items!
  ...
  it('does not expose a custom event property on conversions', function () {
    const items = conversionsProp(schema).items!
    const props = items.properties!
    assert.strictEqual(props.event, undefined, 'event property should be gone')
  })

  it('exposes a udp object with address and port', function () {
    const udp = schema.properties.udp
    assert.ok(udp, 'schema missing udp property')
    assert.strictEqual(udp!.type, 'object')
    const props = udp!.properties!
    assert.ok(props.address, 'missing udp.address')
    assert.ok(props.port, 'missing udp.port')
    assert.strictEqual(props.address!.type, 'string')
    assert.strictEqual(props.port!.type, 'number')
    assert.strictEqual(props.port!.default, 10110)
  })
